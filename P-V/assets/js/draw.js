function test() {
    ...
    var limits = response_data['limits'];
    var points = response_data['points'];

    for (var i = 0; i < limits.length; i++) {
        ctx.fillStyle=limits[i].color;
        ctx.fillRect(limits[i].x,limits[i].y,1,1);

        //console.log(limits[i].color);
    }
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        ctx.arc( (points[i][0] + move) * enlarge , (points[i][1] + move) * enlarge, pointSize, 0, Math.PI * 2, true); // Dibujar un punto usando la funcion arc
...
function draw_voronoi() {
    var data =  localStorage.getItem('response_data');
